...
`streaks`: _(Required)_ If there exists a multiplier for win/loss streaks.\
&emsp;&emsp;&emsp; Options: `Enabled, Disabled`\
`placements`: _(Required)_ If there exists a multiplier for the first 10 matches for placements.\
&emsp;&emsp;&emsp; Options: `Enabled, Disabled`
...
`map_choice`: _(Required)_ Voting or always random.\
&emsp;&emsp;&emsp; Options: `Vote, Random`\
`gamemode_choice`: _(Optional)_ -.\
&emsp;&emsp;&emsp; Options: `Vote, Random`
...
`role`: _(Optional)_ Priority role.
...
`value`: _(Required)_ Whether match outcomes are enabled or not.
...
`seconds`: _(Required)_ Enter the time in SECONDS, or 0 to disable.
