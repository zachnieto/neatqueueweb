...
import { LeaderboardV2Response, LeaderboardPlayer } from '../types';

type PlayerData = LeaderboardPlayer['stats'];
type SortKey = 'mmr' | 'wins' | 'losses' | 'wl' | 'totalgames' | 'streak' | 'peak_mmr' | 'peak_streak' | 'winrate';
...
const Leaderboard = ({ passedGuildId = '', passedChannelId = '' }: LeaderboardProps) => {
    const { guildID, channelID } = useParams();
    
    // Use nuqs for query state management
    const [sortKey, setSortKey] = useQueryState('sort', parseAsString.withDefault('mmr'));
    const [selectedMonth, setSelectedMonth] = useQueryState('month', parseAsString.withDefault('alltime'));
    const [currentPage, setCurrentPage] = useQueryState('page', parseAsInteger.withDefault(1));

    // Local state for column visibility and sort direction
    const [visibleColumns, setVisibleColumns] = useState<SortKey[]>(DEFAULT_COLUMNS);
    ...
    const { data: leaderboardData, isLoading, error } = useQuery({
        ...
        queryFn: () => getLeaderboardV2(guildId, channelId, currentPage, [selectedMonth]),
    ...
    const currentMonthData = useMemo(() => {
        ...
        return leaderboardData.months.find((m: any) => m.month === selectedMonth) || leaderboardData.months[0];
        ...
        setVisibleColumns(prev => {
            if (prev.includes(column)) {
                ...
                return newColumns.sort((a, b) => 
                    AVAILABLE_STATS.indexOf(a) - AVAILABLE_STATS.indexOf(b)
                );
            }
        });
    };
    ...
    const getRankBadge = (rank: number) => {
        if (rank === 1) return { icon: '👑', bg: 'bg-gradient-to-r from-yellow-600/30 to-amber-600/30', border: '' };
        if (rank === 2) return { icon: '🥈', bg: 'bg-gradient-to-r from-gray-400/20 to-slate-400/20', border: '' };
        if (rank === 3) return { icon: '🥉', bg: 'bg-gradient-to-r from-orange-700/20 to-orange-600/20', border: '' };
    ...
    const formatStatValue = (player: LeaderboardPlayer, key: SortKey) => {
        ...
        if (key === 'winrate' && typeof value === 'number') return displayPercent(value);
        if ((key === 'mmr' || key === 'peak_mmr') && typeof value === 'number') return Math.round(value).toString();
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center">
                    ...
                    <p className="text-xl text-gray-300 font-medium">Loading leaderboard...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen py-8 px-4">
            <div className="max-w-7xl mx-auto">
                ...
                <div className={classNames(
                    "grid gap-6",
                    leaderboardData.available_months.length > 1 && leaderboardData.available_months[0] !== 'alltime'
                    ...
                    <div className={classNames(
                        ...
                        <div className="hidden md:block bg-neutral-900 rounded-xl border border-neutral-700 shadow-2xl overflow-hidden">
                            ...
                            <div className="divide-y divide-neutral-800">
                                {sortedPlayers.length === 0 ? (
                                    ...
                                    sortedPlayers.map((player, index) => {
                                        const rank = index + 1 + ((currentPage - 1) * (currentMonthData?.pagination?.per_page || 50));
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div className="w-16 text-center">
                                                            <div className="flex flex-col items-center justify-center">
                                                                {rankInfo.icon && (
                                                                    <span className="text-2xl mb-1">{rankInfo.icon}</span>
                                                                )}
                                                        ...
                                                        {visibleColumns.map((column) => {
                                                            if (column === 'winrate') {
                                                                ...
                                                                return (
                                                                    <div key={column} className="min-w-[80px] flex justify-center">
                                                                        <div 
                                                            ...
                                                            return (
                                                                <div key={column} className="min-w-[80px] text-center">
                            ...
                            {/* Pagination */}
                            {currentMonthData?.pagination && currentMonthData.pagination.total_pages > 1 && (
                                <div className="bg-neutral-800 px-6 py-4 border-t border-neutral-700">
                                    <div className="flex items-center justify-between flex-wrap gap-4">
                                        <div className="text-sm text-gray-400">
                                            Showing <span className="text-white font-semibold">{((currentPage - 1) * currentMonthData.pagination.per_page) + 1}</span> to{' '}
                                            <span className="text-white font-semibold">{Math.min(currentPage * currentMonthData.pagination.per_page, currentMonthData.pagination.total_items)}</span> of{' '}
                                            <span className="text-white font-semibold">{currentMonthData.pagination.total_items}</span> players
                                        </div>
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className="px-4 py-2 text-white font-medium bg-neutral-800 rounded-lg">
                                                {currentPage} / {currentMonthData.pagination.total_pages}
                                            </span>
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <div className="flex flex-col items-center justify-center w-10">
                                                    {rankInfo.icon && (
                                                        <span className="text-lg leading-none">{rankInfo.icon}</span>
                                                    )}
                                                    <span className={classNames(
                                                        'text-xs font-bold',
                                                        rank <= 3 ? 'text-white' : 'text-gray-300'
                            ...
                            {currentMonthData?.pagination && currentMonthData.pagination.total_pages > 1 && (
                                <div className="bg-neutral-800 rounded-xl border border-neutral-700 p-4">
                                    <div className="text-xs text-gray-400 text-center mb-3">
                                        Page {currentPage} of {currentMonthData.pagination.total_pages}
                                    </div>
