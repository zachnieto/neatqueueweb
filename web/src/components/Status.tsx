...
const Status = () => {
  ...
  const [overallStatusColor, setOverallStatusColor] = useState<string>("text-yellow-400");
      ...
      {!loading && (
        <div className="flex flex-col items-center gap-5">
          ...
          <div className="flex justify-around items-center gap-5">
            {statuses.map(status => <div className="text-6xl text-center bg-black/75 p-10 rounded-xl">

              <h1>Node {status.node}</h1>
              <h1 className={status.status === "ONLINE" ? "text-green-400" : "text-red-400"}>{status.status}</h1>
              <h1 className="text-3xl">Shards: {status.shards}</h1>
              <h1 className="text-3xl">Servers: {status.guilds}</h1>
